...
function getDetail(url) {
    ...
    $http.request({
        ...
        handler: function (resp) {
            ...
            console.log({ filmName, filmCover, actresses, filmTime, filmLast, filmEstabName, filmMakerbName, seriesName, directorName, code, screenData });


            LocalData.detail = { filmName, filmCover, actresses, filmTime, filmLast, filmEstabName, filmMakerbName, seriesName, directorName, code, screenData, url };
...
function getActress(url) {
    ...
    $http.request({
        ...
        handler: function (resp) {
            ...
            $("actressView").detail = { birth, age, height, breast, xiong, yao, tun };
...
const testJavbusActress = (detail, actress) => {
    const { birth, age, height, breast, xiong, yao, tun } = detail;
    const { info, shortCode, src, un } = actress;
    ...
    postIFTTT(content, 'javbus_actress')

};


const testJavbusMovie = (detail, actress) => {
    const { birth, age, height, breast, xiong, yao, tun } = detail;
    const { info, shortCode, src, un } = actress;
...
const postIFTTT = (content, key) => {
    console.log(content);
    $http.request({
        method: "POST",
        url: `https://maker.ifttt.com/trigger/${key}/with/key/ba5A6Wvz98s33G_QeCRHub`,
        body: content,
        header: {
...
function favActressButtonTapped(mode, data) {
    if (mode == "add") {
        console.log($("actressView").detail);
        console.log(data);

        testJavbusActress($("actressView").detail, data);
...
function favoriteButtonTapped(mode, data) {
    console.log(LocalData.detail);
    console.log(data);


    if (mode == "add") {

        const { filmName, filmCover, actresses, filmTime, filmLast, filmEstabName, filmMakerbName, seriesName, directorName, code, screenData, url } = LocalData.detail;

        const postData = {
            "Title": `${code} ${filmName} ${filmTime}`, // 注意这个filename不一样
            ...
            "Cover": filmCover,
            "Screen Data": screenData !== 'no' ? screenData.map((i) => (i.link)) : [],
        ...
        postIFTTT(content, 'javbus_movie')
